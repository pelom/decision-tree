...
public class DTreeCrossValidation extends DecisionTreeDefault {
    ...
    public List<Double> validate(final List<Instance> instances, int crossValidationN) throws IOException {
        ...
        for(int i = 0; i < testBundles.size(); i++) {
            ...
            final Node root = construct(caseTest.train);
            //final Map<Integer, Node> resultTestMap = mineInstances(params[TEST_INDEX]);
    ...
    protected int countCorrect(List<Instance> testInstance) {
        ...
        int correct = 0;
        for (final Instance item : testInstance) {
     ...
     * @param k
     */
    public List<List<Instance>> shuffle(final List<Instance> instances, final int k) {
