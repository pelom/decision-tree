...
public class CrossPruningTest {
    ...
    public void crossValidationPruning() throws Exception {
        ...
        final List<Instance> inst = trainInput.getInstanceSet();
        final List<Instance> inst1 = trainInput2.getInstanceSet();

        final List<Double> resultCross = this.treeCross.validate(inst, k);
        System.out.println(OutPrintTree.print(this.treeCross));

        final List<Double> resultPruning = this.treePruning.validate(inst1, k);
